...
GitGalaxy is meant to run *in* CI, not just get starred and forgotten — so instead of treating CI-driven traffic as noise to filter out, we track it as its own adoption signal, side by side with human discovery. Left panel: GitHub stars, forks, unique cloners, and unique profile views. Right panel: GitLab CI/CD Catalog usage (unique projects running it in a pipeline in the last 30 days) and GitHub Action adoption (unique repos referencing `uses: squid-protocol/gitgalaxy` in a workflow, via code search — there's no Marketplace listing yet, so this is the best passive proxy available).
